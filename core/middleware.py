...
class ForceLanguageDebugMiddleware:
    """Debug middleware to see language settings"""
    def __init__(self, get_response):
        self.get_response = get_response
    
    def __call__(self, request):
        # Debug info
        print(f"DEBUG: Language - {translation.get_language()}")
        print(f"DEBUG: Accept-Language header - {request.META.get('HTTP_ACCEPT_LANGUAGE')}")
        print(f"DEBUG: Language cookie - {request.COOKIES.get('django_language')}")
