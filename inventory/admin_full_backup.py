...
from .models import (
    Category, Product, ProductStock, ProductUnit,  # ADD ProductUnit
    Purchase, PurchaseItem, Sale, SaleItem, 
    StockTransfer, TransferBatch, Supplier, 
    RetailStock, RetailSale, Payment,
    PurchaseOrder, PurchaseOrderItem, 
    SaleOrder, SaleOrderItem, CompanyDetails  # ADD CompanyDetails
)
...
# ==================== ADD PRODUCT UNIT ADMIN ====================
@admin.register(ProductUnit)
class ProductUnitAdmin(admin.ModelAdmin):
    list_display = ['product', 'unit_name', 'quantity_in_base', 'selling_price', 'is_default']
    list_filter = ['unit_name', 'is_default']
    search_fields = ['product__name', 'unit_name']
    list_editable = ['selling_price', 'is_default']
    list_per_page = 20
...
class ProductAdmin(admin.ModelAdmin):
    list_display = ['name', 'sku', 'category', 'base_unit', 'cost_price', 'selling_price', 'reorder_level', 'total_stock_display', 'units_count']
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'sku', 'category', 'base_unit')
        }),
        ('Pricing', {
    ...
    def units_count(self, obj):
        ...
        if count > 0:
            return format_html('<span style="color: green;">✅ {} unit(s)</span>', count)
        return format_html('<span style="color: orange;">⚠️ No units</span>')
...
class SaleItemInline(admin.TabularInline):
    model = SaleItem
    extra = 1
    fields = ['product', 'quantity', 'unit_name', 'unit_price_ugx', 'unit_price_cdf', 'total_price', 'total_price_ugx']
    readonly_fields = ['unit_price_cdf', 'total_price', 'total_price_ugx']
    autocomplete_fields = ['product']
...
class SaleAdmin(admin.ModelAdmin):
    list_display = [
        'document_number', 'customer', 'location', 'date', 
        'total_amount', 'paid_amount', 'balance_due', 'document_status'
    ]
    list_filter = ['document_type', 'document_status', 'location', 'date']
    search_fields = ['document_number', 'customer__name']
    readonly_fields = ['document_number', 'created_at', 'updated_at']
    inlines = [SaleItemInline]
    date_hierarchy = 'date'
    list_per_page = 20
    autocomplete_fields = ['customer', 'location', 'created_by']

# ==================== SALE ITEM ADMIN ====================
@admin.register(SaleItem)
class SaleItemAdmin(admin.ModelAdmin):
    list_display = [
        'sale_document', 'product', 'quantity', 'unit_name',
        'unit_price_ugx', 'unit_price_cdf', 'total_price', 'total_price_ugx'
    ]
...
class SaleOrderAdmin(admin.ModelAdmin):
    ...
    autocomplete_fields = ['customer', 'location']
...
class CompanyDetailsAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        # Only allow one company details record
        if CompanyDetails.objects.exists():
            return False
        return True

# ==================== ADMIN SITE CONFIGURATION ====================
admin.site.site_header = "Tusakimu Enterprises - Inventory Management System"
